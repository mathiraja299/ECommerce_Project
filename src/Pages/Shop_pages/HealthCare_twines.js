...
import { allProducts, healthCare } from "../../Data/Products_datas";
...
export default function HealCare_twines(params) {
    const wisht = useSelector((state) => state.Whistlist);

    const HealthCare_data = allProducts.map((item) => {
        const itemInWisht = wisht.find((o) => o.id === item.id);
        return item.category === 'health_care' ? (
        ...
        <>
           ...
           <div className="container">
            ...
            <div className="text-center mt-5 p-2">
                <button className="p-2" style={{ width: '16%', height: '21%' }}> <img src={load} /> Load More </button>
            </div>
