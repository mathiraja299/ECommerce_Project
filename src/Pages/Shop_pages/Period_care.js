...
import { allProducts, periodCare } from "../../Data/Products_datas";
...
export default function Period_care(params) {
    const wisht = useSelector((state) => state.Whistlist);

    const PeriodCare_data = allProducts.map((item) => {
        const itemInWisht = wisht.find((o) => o.id === item.id);
        return item.category === 'period_care' ? (
